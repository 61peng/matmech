...
instruction = open("instruction/hallu_correct.md", "r", encoding="utf-8").read()
system_message = "You are a factuality verification model."
...
def process_mechanism_with_llm(mech_json, all_texts, engine, model_name):
    ...
    for block in mech_json:
        ...
        if exp and "result" in exp and "source" in exp:
            hallu = exp.get("hallu_detection")
            consistency = hallu["consistency"] if isinstance(hallu, dict) and "consistency" in hallu else None
        ...
        if refks:
            for refk in refks:
                hallu = refk.get("hallu_detection")
                consistency = hallu["consistency"] if isinstance(hallu, dict) and "consistency" in hallu else None
                if not consistency:
                    content = f"{refk.get("content")}\n\nReference: {refk.get("source")}"
                    judge_and_write(content, refk.get("source"), refk,
                                    all_texts, engine, model_name)
        ...
        if mech and "description" in mech and "source" in mech:
            hallu = mech.get("hallu_detection")
            consistency = hallu["consistency"] if isinstance(hallu, dict) and "consistency" in hallu else None
            if not consistency:
...
def main():
    ...
    final_dir = f"output_file/{journal}/hallucination_llm"
    out_dir = f"output_file/{journal}/hallucination_correction"
